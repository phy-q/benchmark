...
class Net(nn.Module):
    def __init__(self):
        ...
        self.time_step = C.INPUT_SIZE
        self.ve_feat_dim = C.VE_FEAT_DIM  # visual encoder feature dimension
        self.in_feat_dim = C.IN_FEAT_DIM  # interaction net feature dimension
        self.num_objs = C.MAX_NUM_OBJS
        self.mask_size = C.MASK_SIZE
        # build image encoder
        self.backbone = build_backbone(C.BACKBONE, self.ve_feat_dim, C.INPUT.IMAGE_CHANNEL)

        # extract object feature -> convert to object state
        pool_size = C.ROI_POOL_SIZE
        self.roi_align = RoIAlign(
            (pool_size, pool_size),
            spatial_scale=C.ROI_POOL_SPATIAL_SCALE,
            sampling_ratio=C.ROI_POOL_SAMPLE_R,
        )
        roi2state = [nn.Conv2d(self.ve_feat_dim, self.in_feat_dim, kernel_size=3, padding=1),
                     nn.ReLU(inplace=True)]
        for _ in range(C.N_EXTRA_ROI_F):
            roi2state.append(nn.Conv2d(self.ve_feat_dim, self.in_feat_dim,
                                       kernel_size=C.EXTRA_F_KERNEL, stride=1, padding=C.EXTRA_F_PADDING))
        ...
        for _ in range(C.N_EXTRA_PRED_F):
