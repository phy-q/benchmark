...
from utils.im import get_im_data
from utils.vis import plot_rollouts
from utils.misc import tprint, pprint
from utils.bbox import xyxy_to_rois, xywh2xyxy
...
class PredEvaluator(object):
    def __init__(self, device, val_loader, model, num_gpus, num_plot_image, output_dir):
        ...
        self.model = model
        # input setting
        self.ptrain_size, self.ptest_size = C.PRED_SIZE_TRAIN, C.PRED_SIZE_TEST
        ...
        for batch_idx, (data, _, rois, gt_boxes, gt_masks, valid, g_idx, seq_l) in enumerate(self.val_loader):
            ...
            if self.plot_image > 0:
                ...
                for i in range(rois.shape[0]):
                    batch_size = C.SOLVER.BATCH_SIZE if not C.VAE else 1
    ...
    def _setup_loss(self):
        self.loss_name = []
        self.position_loss_weight = C.POSITION_LOSS_WEIGHT
