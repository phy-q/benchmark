...
class PHYREO(Dataset):
    def __init__(self, data_root, split, image_ext='.jpg'):
        ...
        self.input_size = C.INPUT_SIZE  # number of input images
        self.pred_size = eval(f'C.PRED_SIZE_{"TRAIN" if split == "train" else "TEST"}')
        self.seq_size = self.input_size + self.pred_size
        self.input_height, self.input_width = C.INPUT_HEIGHT, C.INPUT_WIDTH
    ...
    def __getitem__(self, idx):
        ...
        masks = np.zeros((len(images), num_objs, C.MASK_SIZE, C.MASK_SIZE))

        init_image = cv2.resize(images[0], (C.INPUT_WIDTH, C.INPUT_HEIGHT), interpolation=cv2.INTER_NEAREST)
            ...
            if simple_parse:
                for o_id, raw_obj_id in enumerate(obj_ids):
                    ...
                    masks[im_id, o_id] = cv2.resize(
                        mask[y1:y2 + 1, x1:x2 + 1].astype(np.float32), (C.MASK_SIZE, C.MASK_SIZE)
                    ) >= 0.5
                    x1 *= (C.INPUT_WIDTH - 1) / (im_width - 1)
                    x2 *= (C.INPUT_WIDTH - 1) / (im_width - 1)
                    y1 *= (C.INPUT_HEIGHT - 1) / (im_height - 1)
                    y2 *= (C.INPUT_HEIGHT - 1) / (im_height - 1)
        ...
        if random.random() > 0.5 and self.split == 'train' and C.HORIZONTAL_FLIP:
            boxes[..., [0, 2]] = self.input_width - boxes[..., [2, 0]]
            data = np.ascontiguousarray(data[..., ::-1])
            gt_masks = np.ascontiguousarray(gt_masks[..., ::-1])

        if random.random() > 0.5 and self.split == 'train' and C.VERTICAL_FLIP:
        ...
        g_idx = []
        for i in range(C.MAX_NUM_OBJS):
            for j in range(C.MAX_NUM_OBJS):
                if j == i:
                    continue
                g_idx.append([i, j, (i < num_objs) * (j < num_objs)])
        g_idx = np.array(g_idx)

        valid = np.ones(C.MAX_NUM_OBJS)
        valid[num_objs:] = 0
        boxes = np.concatenate([boxes] + [boxes[:, :1] for _ in range(C.MAX_NUM_OBJS - num_objs)], axis=1)
        gt_masks = np.concatenate([gt_masks] + [gt_masks[:, :1] for _ in range(C.MAX_NUM_OBJS - num_objs)],
                                  axis=1)
        ...
        gt_boxes = xyxy2xywh(gt_boxes.reshape(-1, 4)).reshape((-1, C.MAX_NUM_OBJS, 4))
