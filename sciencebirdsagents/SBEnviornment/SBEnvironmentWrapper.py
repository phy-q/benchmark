...
class SBEnvironmentWrapper:
    ...
    def make(self, agent, if_first_server=True, start_level=1, state_representation_type='symbolic'):
        ...
        if if_first_server:
            self.__start_server(self.if_head)
            time.sleep(5)
        self.agent = agent
    ...
    def step(self, action, batch_gt=False):
        """
    ...
    def close(self):
        os.system("ps -ef | grep \"[0-9] java -jar ./game_playing_interface.jar\" | awk '{ print $2 }' | xargs kill -9 ")
        logger.debug("server terminated")
        os.system("ps -ef | grep \"[0-9] ./9001.x86_64\" | awk '{ print $2 }' | xargs kill -9 ")
        logger.debug("game closed terminated")
